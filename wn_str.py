...
class WeightNorm(Wrapper):
    ...
    def _compute_weights(self):
        ...
        with variable_scope.variable_scope('compute_weights'):
            ...
            self.layer.kernel = tf.Print(self.layer.kernel, [tf.reduce_max(tf.abs(self.layer.g))], 'shit from %s' % self.layer.g.name)
    ...
    def _simple_init(self):
        ...
        def _do_init():
            assign_ops = [self.layer.g.assign(self._init_norm(self.layer.v))]
    ...
    def _data_dep_init(self, inputs):
        ...
        def _do_init():
            ...
            with variable_scope.variable_scope('data_dep_init'):
                ...
                scale_init = tf.identity(tf.Print(scale_init,
                    [tf.reduce_max(tf.abs(m_init)), tf.reduce_max(tf.abs(v_init)), tf.reduce_max(tf.abs(scale_init))],
                    "m, v, s at %s" % self.layer.g.name))

            # Assign data dependent init values
            assign_ops = [tf.Print(self.layer.g.assign(scale_init),
                [tf.reduce_max(tf.abs(self.layer.g)), tf.reduce_max(tf.abs(scale_init))], "assign s at %s" % self.layer.g.name)]
        ...
        with tf.variable_scope('meanOnlyBatchNormalization'):
            ...
            if not self.training:
                ...
                with tf.control_dependencies([moving_mean_op]):
                    bias_op = tf.Print(b - m, [tf.reduce_max(m), tf.reduce_min(m)], "Max Min at %s" % self.layer.g.name)
                    return tf.nn.bias_add(x, bias_op, data_format='NHWC')
    ...
    def build(self, input_shape):
        ...
        if not self.layer.built:
            ...
            if not self.data_init:
                with tf.control_dependencies([self._simple_init()]):
    ...
    def call(self, inputs):
        ...
        inputs = tf.Print(inputs, [tf.reduce_max(tf.abs(inputs))], 'Inputs peak at %s' % self.layer.g.name)
        output = self.layer.call(inputs)
        output = tf.Print(output, [tf.reduce_max(tf.abs(output))], 'output peak at %s' % self.layer.g.name)
